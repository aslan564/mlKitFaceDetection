...
import com.mlkit.demo.GraphicOverlay.Graphic
...
class FaceGraphic constructor(overlay: GraphicOverlay?, private val face: Face) : Graphic(overlay) {
    ...
    override fun draw(canvas: Canvas) {
        ...
        textWidth = textWidth.coerceAtLeast(
            idPaints[colorID].measureText(
                String.format(Locale.getDefault(), "EulerX: %.2f", face.headEulerAngleX)
            )
        )
        textWidth = textWidth.coerceAtLeast(
            idPaints[colorID].measureText(
                String.format(Locale.getDefault(), "EulerY: %.2f", face.headEulerAngleY)
            )
        )
        textWidth = textWidth.coerceAtLeast(
            idPaints[colorID].measureText(
